...
try:
    import redis
except Exception as redis_import_error:
    redis = None
...
class CacheManager:
    ...
    def _initialize_redis(self):
        ...
        if redis is None:
            print(f"[cache] redis package unavailable: {_REDIS_IMPORT_ERROR}")
            print("[cache] falling back to direct queries (no caching)")
            self.enabled = False
            self.redis_client = None
            return
        ...
        try:
            ...
            if redis_url:
                ...
                print(f"[cache] redis connected: {redis_host}:{redis_port}")
    ...
    def get_cached_data(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            print(f"[cache] read error key={key}: {e}")
            return None
    ...
    def set_cached_data(self, key: str, data: Any, ttl_minutes: int = 5) -> bool:
        ...
        try:
            ...
            print(f"[cache] write error key={key}: {e}")
            return False
    ...
    def invalidate_cache(self, pattern: str = "*") -> int:
        ...
        try:
            ...
            print(f"[cache] invalidation error pattern={pattern}: {e}")
            return 0
